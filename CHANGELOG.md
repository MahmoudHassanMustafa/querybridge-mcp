...
- 525b460: **New tool: `column_stats`.** Per-column profiling for a table — null %, distinct count, min/max/avg, and (optionally) the top-N most common values per column.
...
- 35703af: **`tables: string[]` filter on `describe_table`, `get_ddl`, `get_foreign_keys`, `get_indexes`.** Same pattern just shipped on `get_table_stats` — adds the "some" mode an agent reaches for constantly. Backward-compatible: existing `table: "..."` callers keep working unchanged.
