...
| `streaming_query` | Stream a large SELECT to a file on disk. `format` selects the output: `ndjson` (default, line-streaming-friendly), `json` (single JSON-array document), or `csv` (RFC 4180, header from first row). Bypasses `execute_query`'s 1k-row in-memory cap. Row + byte caps both apply; hitting either issues `KILL QUERY` on the worker and marks the result `truncated`. Atomic rename on success; temp file is unlinked on failure |
...
querybridge-mcp/
  src/
    ...
    tools/
      ...
      migration-tools.ts  generate_migration (advisory-only DDL emitter)
